...
**Key daemons**:
- `pacemaker-pacemakerd` - Daemon launcher/supervisor
- `pacemaker-controld` - Cluster Resource Manager (CRM) controller, handles fencing integration
- `pacemaker-schedulerd` - Policy Engine, calculates optimal cluster state
- `pacemaker-fenced` - STONITH/fencing daemon, executes BMC power operations
- `pacemaker-execd` - Local Resource Manager, executes OCF agents
- `pacemaker-attrd` - Distributes node attributes across cluster
- `pacemaker-based` - CIB (Cluster Information Base) manager

**Key TNF code paths**:
- `daemons/fenced/` - Fencing daemon implementation
  - `fenced_commands.c` - Fencing operation execution
  - `fenced_remote.c` - Remote fencing delegation
- `daemons/controld/` - CRM controller
  - `controld_fencing.c` - Fencing request/handling integration
  - `controld_corosync.c` - Corosync event handling
  - `controld_membership.c` - Node membership management
- `lib/cluster/` - Cluster layer libraries
  - `corosync.c` - Corosync integration
  - `membership.c` - Node membership and quorum tracking
  - `election.c` - DC (Designated Coordinator) election
- `lib/fencing/` - Fencing API library
- `lib/pacemaker/` - Scheduler/Policy Engine library
  - `pcmk_sched_fencing.c` - Fencing action scheduling

**Critical TNF configuration options** (set in CIB):
```
stonith-enabled:           true     # Required for TNF
stonith-action:            reboot   # Power off then on
no-quorum-policy:          fence    # CRITICAL: fences minority partition
cluster-recheck-interval:  15s      # Default recheck interval
dc-deadtime:               20s      # DC heartbeat timeout
stonith-timeout:           120s     # STONITH operation timeout
stonith-max-attempts:      10       # Retry limit
priority-fencing-delay:    30s      # Delay to prefer fencing one node
startup-fencing:           true     # Fence during startup
```

**Key concepts**:
- **CIB**: Cluster Information Base - stores all cluster configuration
- **DC**: Designated Coordinator - the node making cluster decisions
- **PE**: Policy Engine - calculates transitions to reach desired state
- **Transition Graph**: Ordered action plan generated by scheduler
...
- *Pacemaker Explained* - Configuration reference (essential for TNF)
- *Clusters From Scratch* - Setup guide
